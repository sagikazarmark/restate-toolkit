...
[![docs.rs](https://img.shields.io/docsrs/restate-e2e-harness?style=flat-square)](https://docs.rs/restate-e2e-harness)

**An end-to-end test harness for Restate Rust SDK endpoints against a real server.**

`restate-e2e-harness` provides the building blocks a test suite composes to test
[`restate_sdk`](https://docs.rs/restate-sdk) endpoints against a real `restate-server`,
rather than a suite of its own. **Unix only.**
...
which the Restate image carries at `/usr/local/bin/restate-server`. On Linux, copy it out of the image (Docker
selects the host architecture):
...
See the [repository README](../../README.md#development) for the shared checks.
